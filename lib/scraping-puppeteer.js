...
async function createPage(browser) {
    ...
    page.setDefaultTimeout(3 * 60 * 1000); // 3 mins
...
async function _SMF_scraping(options) {
    ...
    try {
        ...
        if (em !== null && pw !== null && btn !== null) {
		    await Promise.all([
		        page.type('#inpEm', loginData.id),
		        page.type('#inpPw', loginData.pw),
		...
		return await page.evaluate(async () => {
			...
			$('#utTableRoot').find('table > tbody > tr').each((_i, el) => {
...
async function _AIR_scraping(options) {
	const pttUrl = 'https://airtw.moenv.gov.tw/cht/Query/StationData.aspx';
    if (!puppeteerObj.browser || !puppeteerObj.page) puppeteerObj = await _init(options);
    const page = puppeteerObj.page;
	try {
		...
		// 新站頁面已有 jQuery；舊版從 ajax.googleapis.com 跨網域 fetch 再注入的方式，
		// 在 goto 之後會因頁面 origin 不同而被擋（第二次抓取必 Failed to fetch），故改為直接等待
        ...
        // 查詢結果由伺服器端 AsyncPostBack 非同步渲染，等表格真的出現再抓
        await page.waitForFunction(() =>
            document.querySelectorAll('#CPH_Content_UpdatePanel4 table tbody tr').length > 0);
        // 新站結果表格在 UpdatePanel4 內（舊選擇器 CPH_Content_div_table 已不存在）
        return await page.evaluate(() => {
            let result = [];
            for (const el_tr of document.querySelectorAll('#CPH_Content_UpdatePanel4 table tbody tr')) {
                const anArea = [...el_tr.querySelectorAll('td')].map(el_td => el_td.textContent.trim());
                if (!anArea.length) continue;
                result.push({
                    name:   anArea[0],
                    type:   anArea[1],
                    aqi:    anArea[2],
